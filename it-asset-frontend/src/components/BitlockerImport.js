...
function BitlockerImport({ assetId, onImportSuccess }) {
  ...
  return (
    <div className="p-4 border-t mt-4">
        <h4 className="text-md font-semibold mb-2">Import BitLocker Keys from .txt</h4>
        <div className="flex items-center space-x-2">
            <input type="file" accept=".txt" onChange={handleFileChange} />
            <button onClick={handleUpload} className="bg-purple-500 text-white font-bold py-2 px-4 rounded-md hover:bg-purple-600 transition">
                Import
            </button>
        </div>
