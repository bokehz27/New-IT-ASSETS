// src/components/EditSwitchModal.js
import React, { useState, useEffect, useMemo } from "react";
import api from "../api";
import SearchableDropdown from "./SearchableDropdown";

function EditSwitchModal({ sw, onClose, onSwitchUpdated }) {
  const [formData, setFormData] = useState({ name: "", model: "", port_count: 0, ip_address: "", rackId: "" });
  ...
  useEffect(() => {
    api.get("/racks")
      ...
      .catch(err => console.error("Failed to fetch racks", err));
  }, []);
  ...
  const rackOptions = useMemo(
    () => racks.map((r) => ({ value: r.id, label: r.location ? `${r.name} (${r.location})` : r.name })),
    [racks]
  );
    ...
    <div className="fixed inset-0 bg-slate-900/70 flex justify-center items-center z-[9990]">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-[#0d47a1] via-[#1976d2] to-[#2196f3] text-white">
          <h2 className="text-lg font-semibold">Edit Switch</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <input type="number" name="port_count" value={formData.port_count} onChange={handleChange} placeholder="Number of Ports" className="w-full p-2 border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#1976d2] focus:border-[#1976d2] transition sm:text-sm disabled:bg-slate-50 disabled:text-slate-500 disabled:cursor-not-allowed" disabled title="Port count cannot be changed after creation." />
          ...
          <div>
            ...
            <SearchableDropdown options={rackOptions} value={formData.rackId} onChange={(val) => setFormData((p) => ({ ...p, rackId: val }))} placeholder="--- Select a Rack ---" idPrefix="dd-edit-rack" menuZIndex={10000} />
          </div>
