...
const MasterDataManagement = ({ apiEndpoint, title, dataColumns }) => {
  ...
  return (
    <div>
      ...
      <DataTable ref={dt} value={data} paginator rows={10} rowsPerPageOptions={[10, 20, 50, 100]} dataKey="id" size="small" stripedRows showGridlines globalFilter={globalFilter} globalFilterFields={dataColumns.map(col => col.key)} emptyMessage={`No ${title} found.`}>
        {dataColumns.map(col => <Column key={col.key} field={col.key} header={col.name} sortable style={{ minWidth: '12rem' }} />)}
        <Column body={actionBodyTemplate} style={{ width: '8rem', textAlign: 'center' }} header="Actions" />
      </DataTable>

      {/* ✨ 2. ปรับการแสดงผล Dialog */}
      <Dialog
        // header={...} เอา header เดิมออกเพื่อสร้างเอง
        visible={isModalOpen}
        style={{ width: '450px' }}
        footer={modalFooter}
        onHide={closeModal}
        // contentClassName="p-0" เอา padding ของ content ออกไปก่อน
        className="shadow-xl"
      >
        {/* สร้าง Header และ Content ของเราเอง */}
