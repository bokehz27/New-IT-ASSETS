...
const transferableFields = [
  ...
  { key: "pa", label: "PA (Printer Area)" },
  { key: "prt", label: "PRT (Printer No.)" },
];
...
function ReplaceAssetModal({ asset, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.error || "An error occurred during replacement.";
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg flex flex-col overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-slate-600">
            Old Asset:{" "}
            <strong className="text-slate-800">{asset.asset_name}</strong>
          </p>

          <div>
            <label
              htmlFor="new-asset-name"
              className="block text-sm font-medium text-slate-700 mb-1"
            >
              New Asset Name <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              id="new-asset-name"
          ...
          <div className="space-y-3 pt-2">
            <label className="block text-sm font-medium text-slate-700">
              Select data to MOVE to the new asset
            </label>
