...
function SwitchDetailPage() {
  ...
  if (error) return <div className="text-center p-8 text-red-500 bg-red-100 rounded-lg">{error}</div>;

  return (
    <div className="space-y-6">
      {/* --- REDESIGNED HEADER --- */}
      <div>
        <Link to="/switches" className="text-sm font-medium text-indigo-600 hover:text-indigo-800 transition-colors">
          &larr; Back to switch infrastructure
        </Link>
        <div className="mt-2 p-4 bg-white rounded-lg shadow-md border border-gray-200 flex items-center gap-6">
            <div className="flex-shrink-0">
                <SwitchIcon />
            </div>
            <div>
                <h1 className="text-2xl font-bold text-gray-800">{sw.name}</h1>
                <div className="flex items-center gap-x-6 gap-y-1 text-sm text-gray-600 flex-wrap">
      ...
      {/* Component to manage ports remains the same */}
